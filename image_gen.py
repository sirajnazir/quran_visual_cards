...
def _build_hero_block(visual_type="sequential_chain", card_data=None):
    ...
    base_block = (
        f"\n\nHERO CHARACTER — 'ADAM' — SAME person on EVERY card (black ink + accent color ONLY):\n"
        f"This is a SPECIFIC recurring character named Adam. He must look IDENTICAL on every card.\n\n"
        f"ADAM'S FACE (FIXED — never changes between cards):\n"
        f"- Young Muslim man, early 20s. Oval face, pointed chin, sharp jawline\n"
        f"- HAIR: Jet-black solid ink fill, parted on LEFT side, swept RIGHT across forehead. Thick wavy tousled strands\n"
        f"- EYES: Large almond-shaped, dark brown iris. Thick upper lashes. One white sparkle highlight dot per eye\n"
        f"- EYEBROWS: Thick arched brows — LEFT brow sits HIGHER than right brow. This asymmetry is his KEY IDENTIFIER\n"
        f"- NOSE: Small straight angular ink line\n"
        f"- BEARD: Short neat trimmed stubble — fine black stipple dots along jawline + chin + upper lip forming connected outline. Jawline shape visible through it. NOT bushy\n"
        f"- SKIN: BLANK WHITE paper with BLACK INK outlines ONLY — like Tintin or manga. NO skin color, NO beige, NO tan, NO shading on skin\n\n"
        f"ADAM'S HOODIE (FIXED — same design every card):\n"
        f"- Color: Dark charcoal gray base\n"
        f"- CHEST: One LARGE golden (#C8902E) 8-pointed Islamic star centered on chest (~5cm diameter)\n"
        f"- SHOULDERS: Small golden arabesque scroll motifs on each shoulder cap\n"
        f"- BACK: Horizontal golden geometric tessellation band across upper back\n"
        f"- DETAILS: Hood DOWN always, drawstrings hanging visible, front kangaroo pocket\n"
        f"- The golden patterns use the card's accent color flat wash — no gradients\n\n"
        f"ADAM'S OTHER CLOTHING (FIXED):\n"
        f"- PANTS: Dark indigo slim-fit jeans, tapered at ankles\n"
        f"- SHOES: White low-top canvas sneakers with golden (#C8902E) laces\n"
        f"- BUILD: Slim, average height, slightly large head (3D cartoon proportions ~1.2x)\n\n"
    )
    ...
    base_block += (
        f"\nADAM IDENTITY CHECK — before rendering, confirm ALL of these:\n"
        f"  [x] Hair: BLACK, parted LEFT, swept RIGHT\n"
        f"  [x] Left eyebrow HIGHER than right\n"
        f"  [x] Short stipple beard on jawline\n"
        f"  [x] Charcoal hoodie with LARGE gold 8-pointed star on chest\n"
        f"  [x] White skin = blank paper, NO color fill\n"
        f"  [x] White sneakers with gold laces\n\n"
